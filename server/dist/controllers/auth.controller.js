...
export const register = asyncHandler(async (req, res) => {
    ...
    if (profileImglocalFile) {
        uploadResult = await uploadToCloudinary(profileImglocalFile.path);
    }
    ...
    if (!newUser)
        throw new ApiError(500, "Oops! something went wrong, Couldn't create a user");
...
export const updateProfilePic = asyncHandler(async (req, res) => {
    ...
    if (!profileImglocalFile) {
        return res
            .status(400)
            .json(new ApiResponse(400, "New profile image is not provided"));
    }
    ...
    if (profileImglocalFile) {
        try {
            const uploadResult = await uploadToCloudinary(profileImglocalFile.path);
